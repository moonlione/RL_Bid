...
from src.PG.No_threshold.RL_brain import PolicyGradient
from src.PG.No_threshold.RL_brain import store_para
import src.PG.No_threshold.run_this_for_test as r_test
import numpy as np
import pandas as pd
import copy
import datetime
from src.PG.No_threshold.config import config

def run_env(budget, auc_num, budget_para):
    env.build_env(budget, auc_num)  # 参数为训练集的(预算， 预期展示次数)
    # 训练
    print('data loading')
    train_data = pd.read_csv("../../../data/train_fm_embedding.csv", header=None).drop([0])
    ...
    for episode in range(config['train_episodes']):
        ...
        if (episode + 1) % 10 == 0:
            ...
            if max == test_clks_array[len(test_clks_array) - 1:len(test_clks_array)][0]:
                print('最优参数已存储')
                store_para(RL.policy_net)  # 存储最大值
