...
def run_env(budget, auc_num, budget_para):
    train_data = pd.read_csv('data/train_DRLB.csv', header=None).drop([0])
    ...
    for episode in range(config['train_episodes']):
        ...
        for t in range(96):
            ...
            if t >= config['observation_size'] and (t + 1) % config['batch_size'] == 0: # 控制更新速度
...
def run_test(budget, auc_num, optimal_lamda, budget_para):
    test_data = pd.read_csv('data/test_DRLB.csv', header=None).drop([0])
