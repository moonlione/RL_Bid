...
def run_env(budget, budget_para):
    ...
    eCPC = choose_eCPC(config['campaign_id'])
    ...
    e_results_df.to_csv('result_profit/train_episode_results_' + str(budget_para) + '.csv')
    ...
    test_records_df = pd.DataFrame(data=test_records,
                                   columns=['profits', 'budget', 'cost', 'clks', 'real_clks', 'bids', 'imps', 'cpm',
                                            'break_time_slot'])
    test_records_df.to_csv('result_profit/test_episode_results_' + str(budget_para) + '.csv')
