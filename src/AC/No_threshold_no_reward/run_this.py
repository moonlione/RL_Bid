from src.AC.No_threshold_no_reward.env import AD_env
from src.AC.No_threshold_no_reward.Actor import Actor, store_para_a
from src.AC.No_threshold_no_reward.Critic import Critic, store_para_c
import numpy as np
import pandas as pd
import copy
import datetime
...
def para_store_iter(test_results):
    max = 0
...
def run_env(budget, auc_num, budget_para):
    ...
    for episode in range(config['train_episodes']):
        ...
        for i in range(len(train_data)):
            ...
            # RL代理根据状态选择动作)
            action = actor.choose_action(state_deep_copy)

            # 获取剩下的数据
            # 下一个状态的特征（除去预算、剩余拍卖数量）
            if i != len(train_data) - 1:
                auc_data_next = train_data[i + 1: i + 2, :].astype(float).flatten().tolist()[
            ...
            td_error = critic.learn(state_deep_copy, reward, state_next_deep_copy)
            actor.learn(state_deep_copy, action, td_error)
        ...
        if (episode + 1) % 10 == 0:
            ...
            max = para_store_iter(test_clks_array)
            if max == test_clks_array[len(test_clks_array) - 1:len(test_clks_array)][0]:
                print('最优参数已存储')
                store_para_a(actor.actor)  # 存储最大值
                store_para_c(critic.critic)  # 存储最大值
...
def test_env(budget, auc_num, budget_para):
    ...
    for i in range(len(test_data)):
        ...
        # RL代理根据状态选择动作
        action = actor.choose_best_action(state_deep_copy)
...
if __name__ == '__main__':
    env = AD_env()
    actor = Actor(feature_nums=config['feature_num'], action_nums=env.action_numbers, learning_rate=config['learning_rate_a'])
    critic = Critic(feature_nums=config['feature_num'], action_nums=env.action_numbers, reward_decay=config['reward_decay'], learning_rate=config['learning_rate_c'])
